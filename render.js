(function () {
  ...
  function drawPlayer(ctx, p, frame, COLORS) {
    ...
    if (p.swingTimer > 0) {
      const len = 16, sx = p.facing === 1 ? p.x + p.w + 2 : p.x - len - 2, sy = p.y + 4;
      ctx.fillStyle = '#e5e7eb'; ctx.fillRect(sx, sy, len, 3);
    }
  }
  ...
  function drawStructure(ctx, s, COLORS) {
    ...
    if (s.type === 'chair') {
      ...
      ctx.fillRect(s.x + 2, s.y + 2, s.w - 4, s.h - 10); // back
      ctx.fillRect(s.x + 2, s.y + s.h - 8, s.w - 4, 6);   // seat
    } else if (s.type === 'sofa') {
      ctx.fillStyle = '#059669';
      ctx.fillRect(s.x + 2, s.y + s.h - 10, s.w - 4, 8);  // seat
      ctx.fillRect(s.x + 2, s.y + 2, 8, s.h - 6);         // left arm
      ctx.fillRect(s.x + s.w - 10, s.y + 2, 8, s.h - 6);  // right arm
    } else if (s.type === 'bed') {
      ctx.fillStyle = '#fca5a5';
      ctx.fillRect(s.x + 2, s.y + 2, s.w - 4, s.h - 10); // mattress
      ctx.fillStyle = '#7f1d1d';
      ctx.fillRect(s.x + 2, s.y + s.h - 10, s.w - 4, 8); // frame
    }
  ...
  function drawBird(ctx, b) { ctx.fillStyle = '#cbd5e1'; ctx.fillRect(b.x - 6, b.y - 3, 12, 6); ctx.fillStyle = '#94a3b8'; ctx.fillRect(b.x - 10, b.y - 2, 4, 4); }

  function drawBuildPreview(ctx, game) {
    const { gx, gy, canPlace } = game.buildCursor;
    const sz = game.getTypeSize(game.buildType);
    const x = gx * game.TILE, y = gy * game.TILE;
    ctx.save(); ctx.globalAlpha = canPlace ? 0.25 : 0.08;
    ...
    ctx.fillStyle = col; ctx.fillRect(x, y, sz.w, sz.h);
    ctx.restore();
  }
