(function(){
  'use strict';
  console.log('[ZD] input.js v11 loaded');

  const Keys = {
    A:'a', D:'d', W:'w', S:'s',
    LEFT:'ArrowLeft', RIGHT:'ArrowRight', UP:'ArrowUp', DOWN:'ArrowDown',
    SPACE:' ', B:'b', E:'e', SHIFT:'Shift',
    J:'j', V:'v', R:'r',
    Y:'y' // Float-to-safety
  };
  ...
  const keymap = {
    ...
    fire:false, bomb:false, shift:false,
    jet:false, melee:false, reset:false,
    float:false // <-- Y key
  };
  ...
  function setup(game, canvas){
    addEventListener('keydown', (e)=>{
      ...
      if(e.key===Keys.SPACE) keymap.fire=true;   // laser
      if(e.key===Keys.B)     keymap.bomb=true;   // bomb
      ...
      if(e.key===Keys.Y) keymap.float=true;      // trigger float
      if(e.key==='1'||e.key==='2'||e.key==='3') game.selectBuildTypeByKey(e.key);
    ...
    // Mouse: always-build (no build mode)
    ...
    canvas.addEventListener('mousedown', ()=>{
      const x = game.buildCursor.gx*game.TILE; const y = game.buildCursor.gy*game.TILE;
      if(keymap.shift){
