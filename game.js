(function(){
  'use strict';
  console.log('[ZD] game.js v17 loaded');
  ...
  class SpatialGrid{ constructor(cell=TILE){ this.cell=cell; this.map=new Map(); }
    key(cx,cy){ return `${cx},${cy}`; }
    insert(aabb,ref){ const x0=Math.floor(aabb.x/this.cell),y0=Math.floor(aabb.y/this.cell);
    ...
    query(aabb){ const out=new Set(); const x0=Math.floor(aabb.x/this.cell),y0=Math.floor(aabb.y/this.cell);
      const x1=Math.floor((aabb.x+aabb.w)/this.cell),y1=Math.floor((aabb.y+aabb.h)/this.cell);
  ...
  class Game{
    constructor(canvas){
      ...
      this.showHelp=false; // clean by default
    }
    ...
    get solidRects(){ const out=[];
      for(const s of this.structures){ if(s.type==='door'&&s.open) continue; if(s.type==='ladder') continue; if(s.type==='sky') continue; out.push({x:s.x,y:s.y,w:s.w,h:s.h}); }
      return out; }
    ...
    updateZombies(){
      ...
      for(const z of this.zombies){
        ...
        for(const s of this.structures){
          ...
          const vOverlap = zr.y < sr.y+sr.h && zr.y+zr.h > sr.y;
    ...
    updateStructures(){
      ...
      for(const s of this.structures){ if(s.type==='sky') continue; const cx=Math.floor(s.x/TILE); if(!byColumn.has(cx)) byColumn.set(cx,[]); byColumn.get(cx).push(s); }
    ...
    render(){
      ...
      // FPS
      ctx.fillStyle='#e5e7eb'; ctx.font='12px monospace'; ctx.fillText(`FPS: ${this.fps}`, uiX, uiY-2);
      // Health
      ...
      ctx.fillText(`Z: ${this.zombies.length}  |  Blocks: ${this.structures.length}  |  Build: ${this.buildType}`, WORLD_W-12, 20);
      ctx.textAlign='left';

      // Minimal hint
      ctx.globalAlpha=0.6; ctx.fillText('H: help', uiX, jy+barH+14); ctx.globalAlpha=1;

      // Optional help panel
      if(this.showHelp){
        const help = 'Move: A/D or ←/→  •  Jump/Climb: W/↑  •  Jetpack: J  •  Sword: V  •  Laser: Space  •  Bomb: B  •  Build: Click (1=Wall, 2=Door, 3=Ladder, 4=Sky)  •  Toggle Door: E  •  Float: Y (30s)  •  Reset: R';
        const pad=10, y=jy+barH+22;
        const w= WORLD_W - uiX*2, h= 48;
        ctx.save();
        ctx.globalAlpha=0.15; ctx.fillStyle='#000'; ctx.fillRect(uiX, y, w, h); ctx.globalAlpha=1;
        ctx.strokeStyle='rgba(255,255,255,0.1)'; ctx.strokeRect(uiX+0.5, y+0.5, w-1, h-1);
        ctx.fillStyle='#cbd5e1'; ctx.font='12px system-ui, sans-serif';
        ctx.fillText(help, uiX+pad, y+28);
        ctx.restore();
      }
    ...
    fireLaser(){ if(this.gameOver) return; const p=this.player, pr=this.getProjectile(); if(!pr) return;
    ...
    fireBomb(){ if(this.gameOver) return; const p=this.player, pr=this.getProjectile(); if(!pr) return;
    ...
    spawnExplosion(x,y,maxR=92){ const ex=this.explosions.find(e=>!e.active); if(!ex) return;
      ex.active=true; ex.x=x; ex.y=y; ex.age=0; ex.r=0; ex.maxR=maxR; ex.life=32; ex.particles.length=0;
      for(let i=0;i<22;i++){ ex.particles.push({ x, y, vx:rnd(-2.6,2.6), vy:rnd(-4.2,-1.0), r:rnd(1.5,3), life:1, color:i%2? '#fb923c':'#fde047' }); } }

    canPlaceAt(x,y,type){ const h=(type==='sky'?this.TILE:this.TILE*2);
      if(y<0||y+h>GROUND_Y) return false; const rect={x,y,w:this.TILE,h};
      for(const s of this.structures){ const r=s.rect(); if(r.x<rect.x+rect.w && r.x+r.w>rect.x && r.y<rect.y+rect.h && r.y+r.h>rect.y) return false; }
      return true; }
    ...
    spawnZombieWave(){ const count=1+Math.floor(Math.random()*3);
      for(let i=0;i<count;i++){ const side=Math.random()<0.5? -30: WORLD_W+30; const z=new Zombie(side, GROUND_Y-25); this.zombies.push(z); } }
  }
